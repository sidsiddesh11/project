...
public class Signup_Test extends Basetest {
    ...
    public Object[][] getSignupData() throws IOException {
        String excelPath = System.getProperty("user.dir") + "/src/test/resources/Signup_TestCases_Strict_Updated.xlsx";
    ...
    public void testSignupFlow(String sNo, String testCaseID, String objective, String preReq, String steps,
        ...
        try {
            ...
            try {
                if (testCaseID.equals("TC023") || testCaseID.equals("TC024")) {
                    // These are negative cases: account should NOT be created
                    if (signupPage.isAccountCreated()) {
                        test.log(Status.FAIL, "❌ BUG: Account created for invalid data in " + testCaseID);
                        Assert.fail("BUG: Account created when it should have been rejected for " + testCaseID);
                    } else {
                        test.log(Status.PASS, "✅ Signup rejected as expected for " + testCaseID);
                    }
                } else {
